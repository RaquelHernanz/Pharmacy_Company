...
public class MainMenu_Client {
	...
	public void mainMenu_Client() {
		...
		clientMenu.setLayout(new MigLayout("fill", "30[]30[]30", "30[]30"));
		FlatLightLaf.setup();
		try {
		...
		clientMenu.setSize(1150, 750);
		clientMenu.getContentPane().setBackground(new Color(12, 11, 61));
		...
		ImageIcon welcomeLogo = new ImageIcon("resources/welcomeClient.png");
		...
		signout.addActionListener(e -> {
			clientMenu.dispatchEvent(new WindowEvent(clientMenu, WindowEvent.WINDOW_CLOSING));
		...
		clientMenu.add(menuPanel, "grow");
		clientMenu.add(panel2, "grow");
	...
	public void purchase() {
		...
		JLabel clientTypeLabel = new JLabel("Products List, No prescription required");
				...
				new SwingWorker<Void, Void>() {
					...
					protected Void doInBackground() {
						try {
							...
							Client clientBuyer = clientmanager.searchClientByEmail(emailText.getText());
							int clientID = clientBuyer.getId();
							medicinemanager.assignMedicinetoClient(clientID, medicine_code, bill_number, chosenStock);
